...
> **Cross-project reference:** `C:\Users\levir\Documents\Projects\ECOSYSTEM.md` — master architecture, schemas, pipelines, runbooks
...
|-------|------|---------|-------|
| `orats_daily` | 8.3M | Options activity baseline | V1 (active) |
...
SignalGenerator.create_signal_async()
    ...
    ↓
SignalFilter.apply() — 12 filter steps:
    1. ETF exclusion (hardcoded list)
    2. Score threshold (>= 10)
    3. RSI < 55 (S5: raised from 50)
    4. Uptrend SMA20 (from TA cache)
    5. SMA50 momentum (from TA cache)
    6. Notional >= per-symbol baseline (intraday_baselines_30m DB)
    7. ADV >= 1,000 (v57: D-1 from orats_daily, bulk cache on startup)
    8. GEX dead zone (S5+GEX: skip 2-5% above gamma flip)
    9. Crowded trade filter (vw_media_daily_features VIEW — mentions < 5, sentiment >= 0)
   10. Sector limit max 2 (master_tickers DB)
   11. Market regime (Alpaca API — SPY snapshot)
   12. Earnings proximity (earnings_calendar DB — reject if earnings within 2 days)
    ↓
...
Exit Rules:
    - Hard stop: -5% (v57: widened from -2%; USE_HARD_STOP=True, checked every 30s via Alpaca positions API)
...
| Table/View | Purpose |
|------------|---------|
| `orats_daily` | ADV filter — D-1 avg_daily_volume (v57, bulk cache) |
...
| Table | Purpose |
|-------|---------|
| `engulfing_scores` | Written by DayTrading scanner (5min timeframe). UNIQUE on symbol, pattern_date, timeframe |
| `paper_trades_log_b` | Account B trade log (extended schema: direction, stop_price, target_price, risk_per_share, limit_order_id, order_submitted_at, pattern_date, candle_range, pattern_strength) |

**Config (`paper_trading/config.py`):**
| Setting | Default | Purpose |
|---------|---------|---------|
| `USE_ACCOUNT_B` | `True` | Enable/disable Account B |
| `ACCOUNT_B_POLL_INTERVAL_SEC` | `30` | Poll engulfing_scores every N seconds |
| `ACCOUNT_B_MAX_RISK_PER_TRADE` | `500.0` | Max dollar risk per trade |
| `ACCOUNT_B_MAX_CANDLE_RANGE` | `0.57` | Max candle range (high-low) for big-hitter filter |
...
- `paper_trading/alpaca_trader.py` — `sell_short()` and `buy_to_cover()` for bearish patterns
- `paper_trading/position_manager.py` — `open_limit_position()`, `check_pending_orders()`, `check_stops_and_targets()`, `cancel_all_pending_limit_orders()`. Direction-aware P&L. `_pending_limit_orders` dict for tracking unfilled limit orders
- `paper_trading/dashboard.py` — Account B tabs: Signals (Direction/Entry/Stop/Target/Risk/Strength), Positions (Direction/Entry/Current/Stop/Target/P&L%), Closed (Direction/Entry/Exit/P&L%/$P&L/Reason)
- `paper_trading/main.py` — `_poll_account_b_patterns()` runs on 30s interval, `_check_account_b_pending()` checks fills/expiry, `_async_exit_b()` handles stop/target exits

**Rollback:** Set `USE_ACCOUNT_B = False` in config.py, redeploy. Account A continues unaffected.

### Account C — Cameron B2 Pattern Trader (v61+)

Third paper trading account using **Cameron B2 filter stack** (E2E backtest: Sharpe 2.51, WR 57.3%, PF 1.49, N=532). Fully independent of Accounts A/B and the UOA firehose.

**Signal flow:**
```
Pre-market:
  CameronScanner.load_candidates() → orats_daily
    → gap >= 4%, rvol >= 10x, $1-$20, cap 30 symbols

9:45-11:00 AM ET (every 60s):
  CameronScanner.scan_tick()
    → Fetch 5-min bars from Alpaca REST (batch, SIP feed)
    → Run 3 detectors: consolidation_breakout, vwap_reclaim, bull_flag
    → Filter: moderate strength only
    → UPSERT to cameron_scores table

Every 30s:
  CameronChecker.poll_qualifying_patterns()
    → Query cameron_scores (last 10 min, moderate-only)
    → Priority sort: consol_breakout (0) > vwap_reclaim (1) > bull_flag (2), then rvol DESC
    → Bull flag daily cap (max 1/day)
    → Submit limit buy at entry_price (bullish only — Cameron is long-only)
    → Monitor fill (cancel after 30 min if unfilled)
    → Monitor price vs stop_loss and target_1 (via WebSocket, REST fallback)
    → Exit at stop (market), target (market), or EOD 3:55 PM
...
| Variable | Purpose |
|----------|---------|
| `ALPACA_API_KEY_C` | Account C Alpaca paper API key |
| `ALPACA_SECRET_KEY_C` | Account C Alpaca paper secret key |
...
| Setting | Default | Purpose |
|---------|---------|---------|
| `USE_ACCOUNT_C` | `True` | Enable/disable Account C |
| `CAMERON_RVOL_MIN` | `10.0` | Min relative volume for candidates |
| `CAMERON_SCAN_START` | `9:45 AM` | Start scanning patterns |
| `CAMERON_SCAN_END` | `11:00 AM` | Stop scanning patterns |
| `CAMERON_POLL_INTERVAL_SEC` | `30` | Poll cameron_scores every N seconds |
| `CAMERON_SCAN_INTERVAL_SEC` | `60` | Run scanner every N seconds |
| `CAMERON_MAX_BF_PER_DAY` | `1` | Max bull flag trades per day |
| `CAMERON_MAX_RISK_PER_TRADE` | `500.0` | Max dollar risk per trade |
| `CAMERON_MAX_POSITIONS` | `5` | Max concurrent Cameron positions |
...
### Key Data Facts (updated 2026-02-24)
| Source | Coverage | Notes |
|--------|----------|-------|
| Signal JSON (D: drive) | 2023-02-01 → 2026-01-28, 2.3M signals | Has call_pct, trend — NO RSI |
| Replayed signals JSON (D: drive) | 2020-01-02 → 2022-12-30, 1.3M signals | 5-factor scored from Polygon options replay |
| `orats_daily` | Full 6 years (2020-present) | Has stock_price, price_momentum_20d — NO RSI, NO SMA50 |
| `orats_daily_returns` | 2020-01-02 → 2026-01-22 | Join on `ticker` + `trade_date`; decimals (0.01=1%) |
| `ta_daily_close` | 2025-07-30+ only | rsi_14, sma_20, sma_50 — only 7 months |
| Stock minute bars | 2020-2026, 1,536 files | `D:\polygon_data\stocks\*.csv.gz` — 2020-2021 from Alpaca SIP, 2022+ from Polygon |
| Options trades | 2020-2026, 1,529 files | `D:\polygon_data\options\*.csv.gz` — Polygon S3 flat files |
| Stock minute Parquet cache | 2020-2026, 2.12B rows, 30 GB | `D:\backtest_cache\stock_minutes.parquet` — rebuilt 2026-02-24 |
...
|-----------|----------|-------|
| Total repo | ~5GB (polygon_data on D: via junction) | Contains backups |
...
| File/Dir | Size | Fix |
|----------|------|-----|
| `polygon_data/` | NTFS junction → `D:\polygon_data` (~100GB) | Already in .dockerignore |
