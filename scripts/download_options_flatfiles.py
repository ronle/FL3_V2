...
def list_remote_files(s3, year: int, month: int) -> list:
...
def run_download(s3, *, dry_run: bool = False, year_filter: int = None):
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    existing = get_existing_files()
    print(f"Existing local files : {len(existing)}")
    print(f"Output dir           : {OUTPUT_DIR}")
    print(f"S3 prefix            : {OPTIONS_PREFIX}")
    print(f"Date range           : {START_DATE} -> {END_DATE}")
    if year_filter:
        print(f"Year filter          : {year_filter}")
    print()

    # Build list of year/month combos
    months_to_scan = []
    ...
    for i, (year, month) in enumerate(months_to_scan):
        ...
        for rf in remote_files:
            ...
            if dry_run:
                print(f"  [DRY RUN] {filename}  ({size_mb:.1f} MB)")
                continue
            ...
            if ok:
                ...
                existing.add(filename)          # avoid re-download within same run
    ...
    print(f"{'='*60}")
    print(f"Months scanned   : {len(months_to_scan)}")
...
def main():
    ...
    print("=" * 60)
    print("POLYGON FLAT FILES — OPTIONS TRADES DOWNLOADER")
    print("=" * 60)
